  ...
  test("constructor with cookies and initPassport set to false initializes without localStorage", async () => {
    ...
    expect(fetchSpy).toHaveBeenLastCalledWith(
      expect.objectContaining({
        ...
        url: `${TEST_BASE_URL}/identify?osdk=web-0.0.0-experimental&o=site&cookies=no&passport=`,
      })
    );
    ...
    expect(fetchSpy).toHaveBeenLastCalledWith(
      expect.objectContaining({
        ...
        url: `${TEST_BASE_URL}/identify?osdk=web-0.0.0-experimental&o=site&cookies=no&passport=PASSPORT`,
      })
    );
  });
  ...
  test("constructor with cookies and initPassport properties not provided initializes with localStorage and cookies", async () => {
    ...
    expect(fetchSpy).toHaveBeenLastCalledWith(
      expect.objectContaining({
        method: "GET",
        bodyUsed: false,
        url: expect.stringContaining("config?osdk=web-0.0.0-experimental&o=site&cookies=yes"),
      })
    );
    ...
    expect(fetchSpy).toHaveBeenLastCalledWith(
      expect.objectContaining({
        method: "POST",
        _bodyText: '["c:a1a335b8216658319f96a4b0c718557ba41dd1f5"]',
        url: `${TEST_BASE_URL}/identify?osdk=web-0.0.0-experimental&o=site&cookies=yes`,
      })
    );
  });
