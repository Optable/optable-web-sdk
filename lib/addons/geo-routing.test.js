...
describe("getGeoRouting", () => {
  test("resolves the edge host for each supported region", () => {
    expect(getGeoRouting("US")).toBe("us.edge.optable.co");
    expect(getGeoRouting("NA")).toBe("us.edge.optable.co");
  ...
  test("exposes the default geo map", () => {
    expect(DEFAULT_GEO_MAP.NA).toBe("us.edge.optable.co");
    expect(DEFAULT_GEO_MAP.US).toBe("us.edge.optable.co");
