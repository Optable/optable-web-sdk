...
describe("tryIdentifyFromParams", () => {
  ...
  test("calls identify with custom prefix when present", () => {
    setURL(
      "http://some.domain.com/some/path?some=query&something=else&oeid=a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3&foo=bar&baz"
    );
    SDK.tryIdentifyFromParams(undefined, "c1");

    const expected = "c1:a665a45920422f9d417e4867efdc4fb8a04a1f3fff1fa07e998e86f7f7a27ae3";
    expect(SDK.identify.mock.calls.length).toBe(1);
    expect(SDK.identify.mock.calls[0][0]).toEqual(expected);
  });

  test("skips maybeValidEID check if custom prefix is present", () => {
    setURL("http://some.domain.com/some/path?some=query&something=else&oeid=abcd1234&foo=bar&baz");
    SDK.tryIdentifyFromParams(undefined, "c2");

    const expected = "c2:abcd1234";
    expect(SDK.identify.mock.calls.length).toBe(1);
    expect(SDK.identify.mock.calls[0][0]).toEqual(expected);
  });
});
