...
describe("refreshStaleUid2s", () => {
  ...
  const stale = (): StaleUid2[] => [{ source: "uidapi.com", ref: STALE_REF }];
  ...
  it("refreshes a stale token end to end and rewrites the refs sidecar", async () => {
    seedCache();
    respondWith(await encryptResponse({ status: "success", body: BODY }));

    await refreshStaleUid2s(config, stale());
  ...
  it("removes the EID and its refs entry on an opt-out", async () => {
    seedCache();
    respondWith(await encryptResponse({ status: "optout" }));
  ...
  it("skips entries without usable ref data", async () => {
    seedCache();

    await refreshStaleUid2s(config, [{ source: "uidapi.com" } as unknown as StaleUid2]);
  ...
  it("does not throw on a network failure and leaves the cache untouched", async () => {
    ...
    await expect(refreshStaleUid2s(config, stale())).resolves.toBeUndefined();
