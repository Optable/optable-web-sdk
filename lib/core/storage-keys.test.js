...
describe("Storage Key Generation", () => {
  ...
  test("generateOISKeys should return correct storage keys", () => {
    const keysWithNodeConfig = generateOISKeys(mockConfig);
    expect(keysWithNodeConfig).toEqual({
      write: ["OPTABLE_OIS_" + encodeBase64("example.com/node1")],
      read: ["OPTABLE_OIS_" + encodeBase64("example.com/node1")],
    });

    const keysWithoutNodeConfig = generateOISKeys({ ...mockConfig, node: undefined });
    expect(keysWithoutNodeConfig).toEqual({
      write: ["OPTABLE_OIS_" + encodeBase64("example.com")],
      read: ["OPTABLE_OIS_" + encodeBase64("example.com")],
    });
  });
  ...
  test("passport keys read the pre-alias host when a host alias applied", () => {
    const aliasedConfig = { ...mockConfig, legacyHostCache: undefined, aliasedFromHost: "old.example.com" };
    const keys = generatePassportKeys(aliasedConfig);
    expect(keys).toEqual({
      write: ["OPTABLE_PASSPORT_" + encodeBase64("example.com/node1")],
      read: [
        "OPTABLE_PASSPORT_" + encodeBase64("example.com/node1"),
        "OPTABLE_PASSPORT_" + encodeBase64("old.example.com/node1"),
        "OPTABLE_PASS_" + encodeBase64("old.example.com/site1"),
        "OPTABLE_PASS_" + encodeBase64("example.com/site1"),
      ],
    });
  });

  test("passport keys read the pre-alias host without a node", () => {
    const aliasedConfig = {
      ...mockConfig,
      node: undefined,
      legacyHostCache: undefined,
      aliasedFromHost: "old.example.com",
    };
    const keys = generatePassportKeys(aliasedConfig);
    expect(keys).toEqual({
      write: ["OPTABLE_PASSPORT_" + encodeBase64("example.com")],
      read: [
        "OPTABLE_PASSPORT_" + encodeBase64("example.com"),
        "OPTABLE_PASSPORT_" + encodeBase64("old.example.com"),
        "OPTABLE_PASS_" + encodeBase64("old.example.com/site1"),
        "OPTABLE_PASS_" + encodeBase64("example.com/site1"),
      ],
    });
  ...
  test("passport keys keep an explicit legacyHostCache alongside the pre-alias host", () => {
    const aliasedConfig = { ...mockConfig, aliasedFromHost: "old.example.com" };
    const keys = generatePassportKeys(aliasedConfig);
    expect(keys.read).toEqual([
      "OPTABLE_PASSPORT_" + encodeBase64("example.com/node1"),
      "OPTABLE_PASSPORT_" + encodeBase64("old.example.com/node1"),
      "OPTABLE_PASS_" + encodeBase64("old.example.com/site1"),
      "OPTABLE_PASSPORT_" + encodeBase64("legacy.example.com"),
  ...
  test("passport keys only ever write the current host", () => {
    const aliasedConfig = { ...mockConfig, aliasedFromHost: "old.example.com" };
    expect(generatePassportKeys(aliasedConfig).write).toEqual([
      "OPTABLE_PASSPORT_" + encodeBase64("example.com/node1"),
    ]);
  });
