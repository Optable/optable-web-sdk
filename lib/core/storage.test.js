...
describe("LocalStorage", () => {
  ...
  test("setLmpid doesn't store anything if disabled", () => {
    const store = new LocalStorage(randomConfig())
    store.setLmpid({ user: [], audience: [] })
    expect(store.getLmpid()).toBeNull()
  })

  test("setLmpid store first private id if enabled", () => {
    const store = new LocalStorage(randomConfig())
    store.setLmpid({ user: [{ provider: "loblawmedia.ca", ids: [{ id: "PIDV1first" }, { id: "PIDV1second" }] }] })
    expect(store.getLmpid()).toBe("PIDV1first")
  })
})
