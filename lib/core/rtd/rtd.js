// prettier-ignore
// Ignore prettier for this file to keep git history. Will remove in a follow up PR.
...
function log(level, message, ...args) {
  ...
  console[logMethod](`[${timestamp}] ${prefix} ${message}`, ...args); // eslint-disable-line no-console
}
...
function liveIntentUID2(ortb2) {
  return ortb2.user?.ext?.eids?.some((eid) => eid.source === 'uidapi.com' && eid.uids.some((uid) => uid.ext?.provider === 'liveintent.com'));
}
...
function buildRTD(options = {}) {
  ...
  return {
    ...
    skipMerge: 
      sessionStorage.optableForceSkipMerge ? () => true 
        : options.skipMerge !== undefined ? options.skipMerge : () => false,
